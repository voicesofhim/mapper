...
> Legacy ContextLab pipeline note: this document describes the original Wikipedia/Khan-style data scripts that still live in the repo for reference. The accelerator project should use `scripts/import_accelerator_dataset.mjs`, `scripts/embed_embeddinggemma.py`, `requirements-embeddinggemma.txt`, and local Turso/libSQL as documented in the root `README.md` and `AGENT_HANDOFF.md`. Do not treat the OpenAI prerequisite below as part of the accelerator embedding path; accelerator embeddings should use local Google EmbeddingGemma.
